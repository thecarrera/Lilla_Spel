...
void DX::OfflineCreation(HMODULE hModule, HWND* wndHandle)
{
	...
	//PARTICLES
	handler.initiateThatThing(this->gDevice, this->gDeviceContext);	//NEW
	handler.CreateShaders(this->gDevice); //NEW

	//col = Collision(this->FBX.getMeshes(), FBX.getMeshCount());	//ta bort för noClip

	//interactiveCol = InteractiveCollision(this->FBX.getMeshes(), FBX.getMeshCount());	//ta bort för noClip
...
void DX::createMenu()
{
	...
	DirectX::XMVECTOR lookAtVec = {0, -1, -0.01f};
...
void DX::Update()
{
	...
	if (this->menuMsg == false)
	{
		...
		//interactiveCol.test(col.getCollisionData(), col, this->SM); //ta bort för noClip
...
void DX::renderInGameMenu()
{
	...
	this->gDeviceContext->PSSetShaderResources(0, 1, &this->gMenuRTV[2]); //kanske 1 i början där?
